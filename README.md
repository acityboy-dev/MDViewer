...
- 사이드바와 문서 헤더를 숨기고 `360×240`까지 축소 가능한 풀 컴팩트 보기 모드 (`Ctrl+Shift+F`)
- 중앙 문서 뷰어, Markdown 편집기, 저장 전 미리보기
- `50%~200%` 문서 확대·축소와 브라우저형 단축키 (`Ctrl++`, `Ctrl+-`, `Ctrl+0`, `Ctrl+휠`)
...
테마, 타이포그래피, 본문 폰트, 문서 배율, 항상 위, 편집 방식, 파일 변경 감지와 언어 선택을 `%APPDATA%\MarkFlow\settings.json`에 통합 저장합니다. 임시 파일 작성 후 교체하는 방식으로 저장하며 다음 실행 시 값을 복원합니다.
...
`Assets/MarkFlowLogo.svg`가 MarkFlow 아이콘의 단일 편집 원본입니다. 빌드 시 `Tools/MarkFlow.IconBuilder`가 SVG를 읽어 16, 20, 24, 32, 40, 48, 64, 128, 256, 1024px PNG와 다중 해상도 `MarkFlow.ico`를 생성합니다.

PNG와 ICO는 빌드 생성물이므로 직접 수정하지 않습니다. SVG 또는 변환기 코드가 변경된 경우에만 MSBuild의 `GenerateMarkFlowIcons` 타깃이 다시 실행됩니다.
